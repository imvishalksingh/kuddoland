...
export default function CartDrawer() {
  ...
  useEffect(() => {
    ...
    if (cartOpen) {
      ...
      html.style.overflow = '';
      body.style.overflow = '';
      setIsCheckingOut(false); // Reset when drawer closes
    }
    return () => { 
      html.style.overflow = '';
      body.style.overflow = '';
    ...
    <>
      <aside className={`cart-drawer ${cartOpen ? "is-open" : ""}`}>
        <div className="cart-header">
          <h2>
            {isCheckingOut ? (
              <button className="back-link" onClick={() => setIsCheckingOut(false)}>
                ← Back to Cart
              </button>
            ) : (
            ...
            <>
              ...
              <div className="cart-totals">
                ...
                <div className="drawer-actions">
                  <button className="btn-coral btn-full" onClick={() => setIsCheckingOut(true)} disabled={items.length === 0}>
                    Proceed to Checkout
                  </button>
                  <button 
            ...
            <div className="checkout-section">
              ...
              <form className="checkout-form" onSubmit={handleCheckoutSubmit}>
                ...
                <button className="btn-coral btn-full" type="submit" disabled={checkoutLoading || items.length === 0}>
                  {checkoutLoading ? "Placing order…" : "Complete Order"}
                </button>
